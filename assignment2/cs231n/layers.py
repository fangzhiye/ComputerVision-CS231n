...
def affine_forward(x, w, b):
    ...
    Inputs:
    - x: A numpy array containing input data, of shape (N, d_1, ..., d_k)N应该是通图片的通道
    - x: 维数是[N, D]
    - w: A numpy array of weights, of shape (D, M)
    - w: 维数是[D,M]即可以看成共有M个神经元，每个神经元的维数是D维
    - b: A numpy array of biases, of shape (M,)

    Returns a tuple of:
    - out: output, of shape (N, M) 每个样本都会有M个输出
    - cache: (x, w, b) 
    """
    out = None
    ###########################################################################
...
def affine_backward(dout, cache):
    ...
    #要保证所有损失函数L梯度的shape都要与其相关的矩阵变量的shape相同
    #例如x的维数要与dx相同
...
def batchnorm_backward(dout, cache):
    ...
    dvar = -0.5 * ((var + eps) ** (-1.5)) * dx_hat_denominator #x**y是返回x的y次幂
            ...
            for c in range(C):
                ...
                for i in range(new_H):
                    for j in range(new_W):
                        conv_newH_newW[i,j] += np.sum(pedded_x[i * stride: i * stride + HH,
                                                              j * stride : j * stride + WW] * w[f,c,:,:])
...
def conv_backward_naive(dout, cache):
    ...
    for n in range(N):
        for f in range(F):
            for i in range(new_H):
                for j in range(new_W):
                    #事实上每做一次卷积的时候，都会加一次bias，这样卷积的结果有多少像素，那么
                    #在计算的时便加了多次bias因为bias的局部梯度为1，这样*dout梯度就会得到最终的
                    #梯度变化
                    db[f] += dout[n,f,i,j] 
                    #卷积核其实也有多个通道，即df和padded的通道数是一样的
                    dw[f] += padded_x[n,:,i*stride:HH + i * stride, j * stride:
                                      WW + j * stride] * dout[n, f, i, j]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    N, C, H, W = x.shape
    #x.shape是[N(0) C(1） H(2) W(3)] 转置之后[N, H, W , C]x每元素的坐标顺序都会发生这样的变化
    x_new = x.transpose(0,2,3,1).reshape(N*H*W,C)#其实是对每个神经元的输进进行正则化
    #例如有C个神经元可以池化，那么N个样输本经过每个神经元后有N*H*W数据，对单个神经元的输出进行正
    #则化
    out, cache = batchnorm_forward(x_new, gamma, beta, bn_param)
    out = out.reshape(N, H, W, C).transpose(0, 3, 1, 2) #后新转为N,C,H,W shape
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
